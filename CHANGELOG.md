...
- **Initial velocities**
  ...
  - `*INITIAL_VELOCITY_GENERATION` → `/INIVEL/AXIS` + a companion `/FRAME/FIX`.
    ...
    to the correct global velocity. `STYP` 0/1/2/3 (all / part set / part /
    node set) scopes the group; `PHASE`, `IVATN`, `IRIGID` and a nonzero `ICID`
    are warned + dropped, and `*INITIAL_VELOCITY_GENERATION_START_TIME` stays
    skipped. Starter-validated (0 errors: NUMFRAM/NINVEL/NUMSKW all ingested).
- **Rigid walls**
