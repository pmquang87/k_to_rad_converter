...
static pre-solve:
  ...
  2. recover each element's axial force  N = (E·A/L)·(u₂−u₁)·ê;
...
* **SHELL and SOLID elements: NOT SUPPORTED — WARNED, NOT WRONG.**  A correct
  membrane/continuum geometric stiffness needs the full element stress tensor
  recovered from the pre-solve and the corresponding K_g integrals; a naive
  version emits *wrong* buckling factors, which is worse than none.  These
  elements are skipped and a prominent warning is printed; if the model is
  shell/solid dominated the tool refuses to report a buckling factor.
...
def assemble_geometric_stiffness(
    ...
    """Assemble K_g (in the exported DOF space) from the beam elements.

    Returns (K_g, counts, axial_forces).  ``counts`` reports how many beam
    elements contributed and how many shell/solid elements were skipped
    (unsupported).  ``axial_forces`` are the recovered per-beam N (tension +).
    """
...
def main(argv: Optional[List[str]] = None) -> int:
    ap = argparse.ArgumentParser(
        ...
        description="Offline linear (eigenvalue) buckling from an OpenRadioss "
                    ...
                    "Beam elements are validated (Euler column); shells/solids "
                    "are warned as unsupported.")
    ...
    print("Assembling geometric stiffness from beam elements ...")
    Kg, counts, axial = assemble_geometric_stiffness(state, mesh, stiff, u)
    print(f"  {counts['beam']} beam element(s) contributed K_g")
    if counts["shell"] or counts["solid"]:
        print(f"  WARNING: {counts['shell']} shell + {counts['solid']} solid "
              "element(s) are UNSUPPORTED for geometric stiffness and were "
              "SKIPPED. Buckling factors reflect the beam sub-structure only; "
              "for shell/solid-dominated models they are NOT valid.")
    if counts["beam"] == 0 or Kg.nnz == 0:
        print("ERROR: no beam geometric stiffness assembled (no supported "
              "elements carried axial force under the reference load). Refusing "
              "to report a buckling factor.", file=sys.stderr)
        return 1
    if axial:
