...
def _warn_section_solid_one_point_ale(state: ConversionState, secid: int,
    ...
    state.warn(
        ...
        "properties emit) - but that gate does NOT fire on this route, and "
        "the claim that it does was withdrawn in round 5: the Isolid these "
        "sections emit is 1, so advection_B converts and runs at 0 ERROR / 0 "
        "WARNING with Iale 1 AND with Iale 2 (sgrtails.F:920-926 passes). "
        "What the remap costs is the ANSWER, measured on that very deck: the "
        ...
        "going 10.0 -> 0.0 is NOT a failure: the tracer leaves a 3000 mm "
        "FIXED domain at 1e5 mm/s in 0.05 s, which is the expected Eulerian "
        "answer.) With Isolid 1 the remap was MEASURED destructive on the "
        "other two carriers too: taylor_B goes from IE +5.1 % / KE +4.9 % "
        "against its LS-DYNA reference to a 99.9 % energy error at 198220 "
        "cycles, and channel_A from IE -98.9 % / KE -25.6 % to -100 % / "
        "-94.3 %. dyna2rad makes the same Lagrangian choice - there is no "
        "Iale anywhere in its conversion tree. A real ALE conversion "
        "additionally needs an "
...
_CONTACT_SPELLING_NOTES = {
    ...
    "twoway": (
        ...
        "The flag is OFF by default because the seven interfaces of the "
        "class that have a measured arm disagree with each other (1 better, "
        "3 worse, 2 byte-inert, 1 mixed) and 8 of the 15 are still "
        "unjudgeable; a default-ON warning names the derived value on every "
        "carrier either way. Set one interface with --inter-gapmin "
        "<id>=VAL."),
    "forming": (
...
RARE_MATERIAL_KEYWORDS = {
    ...
    **{
    ...
    "BOUNDARY_THERMAL_WELD": _thermal_deferred(
        ...
        "no Radioss counterpart THAT MOVES: /IMPFLUX applies its flux "
        "density to a FIXED group (fixflux.F:149-203) with NO POSITION "
        "ARGUMENT, and nothing in the ENGINE moves that group - fixflux.F:210 "
        "is a substitution into the group's own nodes and the starter "
        "rewrites IBFFLUX only at decomposition. Two facts this refusal used "
        "to state are WRONG and are withdrawn: /IMPFLUX DOES have a "
        "volumetric form (a GRBRIC_ID group, radioss2018/LOADS/impflux.cfg:"
        "25/54, taken by fixflux.F:205's VOLUMIC branch), so the DEPTH "
        "distribution is expressible; and it DOES have its own time window "
        "(TSTART/TSTOP and a sensor, hm_read_impflux.F:120/129/130, applied "
        "at fixflux.F:100-115). What survives is the motion: a Goldak source "
        "travelling along a seam would have to be written as N "
        "time-windowed /IMPFLUX VOLUME cards, one per seam segment - about 33 "
        "on 05_1's 425-element seam - which is a synthesis, not a mapping, "
        "and is not implemented. It is also blocked behind ERROR 179 on a "
        "*MAT_THERMAL_*-only part, which every carrier of this keyword hits "
        "first"),
    "BOUNDARY_THERMAL_WELD_TRAJECTORY": _thermal_deferred(
        ...
        "no Radioss counterpart THAT MOVES, for the same reason as "
        "*BOUNDARY_THERMAL_WELD: /IMPFLUX has no position argument and "
        "nothing in the engine moves its group. Its volumetric form and its "
        "own time window DO exist - see that entry for the two withdrawn "
        "claims and for the N-card synthesis this would need. Censused on "
        "F:: 4 cards on 4 keys; 05_1 is IFORM 2, Q 5e6, a 3.0 / b 2.5 / "
        "cf 2.0 / cr 8.0, travelling 6 mm/s for 6.8 s along a 425-element "
        "seam"),
}
