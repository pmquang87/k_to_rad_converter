...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="The /IMPL/QSTAT/DTSCAL inertia-stabilization scale on a "
             ...
             "file, all three not_comparable BOTH WAYS - no benchmark fidelity "
             "is lost, only a status label): 1908 cycles / 10 s NORMAL at 0.1 "
             "against a run still going at the 600 s campaign cap at 10, having "
             "reached t = 0.32 to 0.60 of 1.0 depending on machine load, at "
             "nt 3 AND nt 4 - the verdict does not flip with nt. Pass 0.1 to "
