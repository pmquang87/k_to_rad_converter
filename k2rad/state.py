...
class SectionBeam:
    ...
    # *SECTION_BEAM card 2i (ELFORM=9, spot weld beam), Manual Vol I R17
    # p.41-22/23:  TS1  TS2  TT1  TT2  PRINT  -  ITOFF  -
    #   TS1/TS2  beam thickness (CST=0/2) or OUTER diameter (CST=1) in the
    #            s-direction at node n1 / n2,
    #   TT1/TT2  beam thickness (CST=0/2) or INNER diameter (CST=1) in the
    #            t-direction at node n1 / n2.
    # There is no volume and no area on this card — VOL/INER/CID/CA is card 2f,
    # which belongs to the ELFORM=6 DISCRETE beam.
    ts2: float = 0.0
    tt1: float = 0.0
    tt2: float = 0.0
    # Card 1 field 5 (cols 41-50): cross-section type — 0 = rectangular,
    # 1 = tubular (circular only), 2 = arbitrary (user integration rule).
    cst: int = 0
    # Card 2i field 7: 1 = torsional stiffness of the spot weld is zero.
    itoff: int = 0
