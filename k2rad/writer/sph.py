...
# raises **ERROR 3047**.
#
# This is the complete set of laws that make that call in the 2026-05-20 starter
# tree. dyna2rad imposes NO law filter of its own — its ``GetComponentType``
# does classify SPH parts, but the only two call sites are a MAT_100 spotweld
# test and a computed-then-unused local — so an SPH part there is converted
# exactly as if it sat on a solid and the starter is the first to object.
_SPH_COMPATIBLE_LAWS = frozenset({
    0, 1, 2, 3, 4, 5, 6, 10, 12, 13, 14, 21, 22, 23, 24, 28, 29, 31, 33, 34,
    35, 36, 38, 40, 41, 42, 49, 66, 70, 72, 75, 79, 81, 93, 97, 102, 103, 105,
    109, 121, 126, 128, 129, 131, 133, 163,
})
...
def _split_mixed_family_sections(state: ConversionState) -> None:
    ...
    for secid in sorted(state.sec_sph):
        ...
        state.warn(
            ...
            f"part(s) {others} that carry shells, solids, thick shells or "
            "beams. Both families need a /PROP and only one can own the SECID, "
            f"so the SPH property is emitted as /PROP id {prop_id} and part(s) "
            f"{here} are repointed at it. Writing both under {secid} would be "
            "starter ERROR 79 (DUPLICATE ID IN PID DEFINITION). Give the "
            "particles their own *SECTION_SPH to control the id.")
...
def _interparticle_distance(pts: Sequence[Tuple[float, float, float]]) -> float:
    ...
    makes the answer EXACT rather than "the closest point in the 3x3x3 block":
...
def _decide_mass_route(state: ConversionState, sec: SectionSph, prop: SphProp,
    ...
    if live:
        ...
        prop.mp = (rho * sum(vols) / len(vols)) if (vols and rho > 0.0) else 1.0

    # The smoothing length the LS-DYNA deck asks for, if it can be stated.
    h0, h_source, dref_split = _lsdyna_h0(state, sec, pids, cells_by_pid)
    ...
    prop.h_source = ""
    if h0 > 0.0:
        h_rad = _radioss_h(prop.mp, rho) if rho > 0.0 else 0.0
        why = ("the particles carry DIFFERENT masses"
               if not uniform else "the section's parts disagree about it")
        detail = ""
        if h_rad > 0.0:
            detail = (f" Radioss will use h = (sqrt(2)*{prop.mp:g}/{rho:g})^(1/3)"
                      f" = {h_rad:g} against the deck's {h0:g} — a ratio of "
                      f"{h_rad / h0:.4f}, i.e. {abs(1.0 - h_rad / h0) * 100:.3g} % "
                      + ("smaller" if h_rad < h0 else "larger")
                      + f" support radius and {abs(1.0 - (h_rad / h0) ** 3) * 100:.3g} % "
                      + ("fewer" if h_rad < h0 else "more")
        ...
        state.warn(
            ...
            f"{why}, so each particle has to carry its own mass on its /SPHCEL "
            "row — and a mass-carrying particle makes Radioss DERIVE h from "
            "that mass and IGNORE the property's (spinih.F:85-95)." + detail
            + " The MASS is exact either way; only the smoothing length "
            "differs. Give the particles one uniform mass, or accept the "
            "derived h.")
    elif sec.sphini > 0.0:
        state.warn(
            f"*SECTION_SPH {sec.secid}: SPHINI={sec.sphini:g} is DROPPED. A "
            "particle that carries its own mass makes Radioss derive its "
            "smoothing length from that mass, so the property's h is never "
            "read (spinih.F:90-95).")
    if dref_split:
        state.warn(
    ...
    if n_zero:
        state.warn(
            ...
            f"own, so they take the property's Mp = {prop.mp:g} (the mean of "
            "the particles that DO state one). Their /SPHCEL Flag is written 0, "
            "never 1: a Flag=1 row with a blank MASS is a SILENT ZERO-MASS "
            "particle — measured, TOTAL MASS = 0.000000000000 with no "
            "diagnostic at all.")
...
def _lsdyna_h0(state: ConversionState, sec: SectionSph, pids: List[int],
               cells_by_pid: Dict[int, List[SphCell]]):
    """``(h0, source, per_part_dref)`` — the smoothing length the LS-DYNA deck
    asks for.
    ...
    ``per_part_dref`` is non-empty only when the section's parts DISAGREE — the
    one case where the deck's own rule cannot be expressed by a single property.
    """
    if sec.sphini > 0.0:
    ...
    if hi - lo > _DREF_AGREE_TOL * hi:
        return 0.0, "", drefs
    src = (f"CSLH {sec.cslh:g} x the measured interparticle distance {hi:g}"
           + (" — CSLH left blank, so the manual's default 1.2"
              if sec.cslh_blank else ""))
    return sec.cslh * hi, src, []
...
def _make_sph_properties(state: ConversionState) -> List[str]:
    ...
    for secid in sorted(state.sph_props):
        ...
        lines += [
            ...
            prop.title[:80],
            _PROP_CARD1_HDR,
            (f"{_f(prop.mp)}{_f(_QA_DEFAULT)}{_f(_QB_DEFAULT)}{_f(0.0)}"
             f"{_i(0)}{_i(prop.h_1d)}"),
            _PROP_CARD2_HDR,
...
def _emit_sphcel_block(state: ConversionState, pid: int,
    ...
    for c in cells:
        if per_cell and c.mass > 0.0:
            lines.append(f"{_i(c.nid)}{_i(c.flag)}{_f(c.mass)}")
        else:
