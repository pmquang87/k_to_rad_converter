...
| … its `BOXID`/`INOUT`/`CID` | — | a SPATIAL death applied *"without regard to TIME, IDGRP, or PERCENT"* (Vol I R17 p.17-251); `/ACTIV` is time/sensor-only, so the card is dropped **by name** |
...
| … its `BIRTH` | `Tstart = BIRTH` **and** the curve pre-shifted by `+BIRTH` | LS-DYNA's BIRTH does both (*"the abscissa values are shifted by an amount BIRTH … same effect as OFFA = BIRTH"*), while the Radioss `Tstart` is a pure gate (`fixfingeo.F:155/168`); emitting one without the other runs the wrong part of the curve |
| `*INTERFACE_SPRINGBACK_LSDYNA[_NOTHICKNESS]` | the ENGINE block `/DYNAIN/DT[/ALL]` + `/DYNAIN/SHELL/STRES/FULL` + `/DYNAIN/SHELL/STRAIN/FULL` | writes `<root>_NNNN.dynain` in LS-DYNA keyword format (`*NODE`, `*ELEMENT_SHELL_THICKNESS`, `*INITIAL_STRESS_SHELL`, `*INITIAL_STRAIN_SHELL`) — the springback stage reads it straight back into k2rad. `PSID` → the part list, with a `/ALL` fallback because a bare `/DYNAIN/DT` with an empty list is starter `ERROR 1909`; SHELLS ONLY (`fredynain.F` has no other element writer), so a shell-free deck emits nothing instead of a 4-line stub |
...
`0.9·ENDTIM  0.02·ENDTIM` instead — at most six files, the last one on the run's
final cycle. Take the highest-numbered `.dynain`.
